...
> Last updated: 2026-07-09 (Excel-review follow-up batch: BUG-65~69 logged+fixed, BUG-62 fixed)
...
| P2 Medium    | 0 | 25 (BUG-12~20, BUG-29~30, BUG-39~49, BUG-55, BUG-68~69) |
...
| BUG-62 | `agents/job_agent.py` `_process_scraped_jd` (empty-company early return) | **Extraction returning empty `company` leaves no audit row** (PRJ-004 Phase 4 code review F5; pre-existing, not introduced by PRJ-004). When Gemini returned data without a `company`, the JD was skipped with only a log line — no `JSON ERROR` row was written, so the URL was retried on every run with no sheet-visible trace. Fix (2026-07-09): the empty-company branch now stages `(url, "EXTRACTION EMPTY — NOT JSON", hash)`; the non-JSON sentinel trips `json.loads` inside `batch_upsert_jd_records`, which writes the existing `_JD_ERROR_ROW` (company="JSON ERROR", data_quality="failed") — sheet-visible, excluded from matching, still retried next run via the same URL-keyed row (bounded growth). The recommended N-consecutive-failure counter was deliberately skipped as new infrastructure; the immediate audit row achieves observability with exact reuse of the JSON-ERROR mechanism. Added `TestBug62EmptyExtractionAuditRow` (2 tests incl. workbook round trip). | Fixed |
| BUG-64 | `agents/job_agent.py` `_fetch_workday_jobs` | **Workday CXS rejects page size >20 with HTTP 400** — found at the G6a live launch gate: the PRJ-004 pagination used `limit: 50`, which NVIDIA's tenant (and Workday CXS generally) rejects, so the fetch returned 0 postings. Unit tests passed because mocks accepted any limit. Fixed same day: `_PAGE_SIZE=20`, `_MAX_PAGES=250` (guard still 5,000 postings); tests now assert `limit ≤ 20` on every call. Verified live post-fix. | Fixed |
| BUG-63 | `tests/test_acceptance.py` (RunSummary) | **No assertion that RunSummary notes carry the `gemini usage:` entry** (PRJ-004 test analysis P2). The REQ-004-26 trial-run cost gate reads this note; a silent regression in the note-writing hook would only surface at the trial run. Cheap addition to the existing `TestRunSummaryWritten` pattern. | Open |

---

## 2026-07-09 Excel-review follow-up batch

> User's manual review of the post-launch dashboard surfaced 5 bugs, fixed together with BUG-62. Plan: `.claude/plans/lovely-tinkering-bumblebee.md`.

### P0 — Critical

| # | File | Description | Status |
|---|------|-------------|--------|
| BUG-65 | `shared/excel_store.py` `get_jd_url_meta`/`get_jd_urls`; `agents/job_agent.py` `process_company` | **User triage tabs invisible to dedup — next run would undo manual triage.** The user moved all JD_Tracker rows into new `JD_ToApply` / `Skipped JD` tabs; dedup read only JD_Tracker (now empty), so the next run would re-scrape and re-add all ~126 triaged JDs. Fix: `TRIAGE_SHEETS` constant + `get_triaged_jd_urls()` (missing tabs tolerated); URLs in either triage tab are PERMANENTLY excluded from `to_process` and the incomplete-retry list; both tabs auto-created with `JD_HEADERS` by `get_or_create_excel` (existing user rows never touched). ⚠️ Renaming the tabs disables the exclusion — keep the exact names `JD_ToApply` / `Skipped JD`. Tests: `TestGetTriagedJdUrls`, `TestTriagedUrlExclusion`, `TestGetOrCreateExcel` extensions. | Fixed |
...
| # | File | Description | Status |
|---|------|-------------|--------|
| BUG-68 | `shared/firecrawl_pool.py` (new); `agents/job_agent.py` | **Firecrawl credits-exhausted (402) errors were invisible and wasted retries.** All 4 Firecrawl call sites caught `Exception` broadly and logged at debug level; `_firecrawl_map` burned 3 retry delays on 402s that can never succeed; no key pool existed. Fix: `FirecrawlKeyPool` mirroring `gemini_pool` — reads `FIRECRAWL_API_KEY` + `FIRECRAWL_API_KEY_2`, rotates on 402/"insufficient credits"/429, and once ALL keys are quota-failed prints ONE loud console warning (Tavily BUG-44 pattern) then returns None instantly so callers skip straight to free fallbacks (crawl4ai/requests). Non-quota errors re-raise, preserving each call site's retry/fallback semantics. `fc_key` parameter threading removed; the 4 inline `FirecrawlApp` constructions replaced by pool calls. Caveat: 402 detection is string-matched (same class as gemini_pool/Tavily) — verify the text on the first live 402. | Fixed |
| BUG-69 | `shared/excel_store.py`; `agents/company_agent.py` `run_reenrich_business_focus` (new) | **Blank Business Focus never self-heals** (root cause of the user's "missing Company_List info" — Tavily quota, NOT Firecrawl: company_agent doesn't use Firecrawl at all). Blank Career URLs are backfilled every run by `run_phase_1_5`, but blank/N-A Business Focus had no detector. Fix: `get_incomplete_company_rows()` + `update_company_business_focus()` (mirror the JD-side pattern); `run_reenrich_business_focus()` runs in `main()` after Phase 1.5 — one Tavily context search per blank row (quota error aborts gracefully, BUG-44 pattern) + one batched Gemini call (migrate_tracks pattern, new `BusinessFocusItem/List` schemas); hallucinated names are never written. | Fixed |
| BUG-70 | `shared/tavily_pool.py` (new); `agents/company_agent.py`; `agents/job_agent.py` | **Tavily plan-limit errors evaded every quota-abort check.** Tavily's real error text is "This request exceeds your plan's set usage limit" — it contains none of `402`/`429`/`quota`, so the BUG-44 substring checks in `discover_ai_companies` / `_tavily_extract_career_url` / `_find_workday_url` / `run_reenrich_business_focus` treated it as a transient warning and the 2026-07-09 04:00 run spun through hundreds of doomed Tavily calls (visible in `logs/pipeline-20260709-040151.log`). Fix: `TavilyKeyPool` (mirror of BUG-68's `FirecrawlKeyPool`) — `TAVILY_API_KEY` + `TAVILY_API_KEY_2`, rotation on 402/429/quota/rate-limit/**usage-limit/exceeds-your-plan**; once ALL keys quota-fail, one loud console warning then instant `TavilyQuotaExhausted` whose message contains "429"+"quota" so every existing call-site check recognizes it unchanged (Tavily callers have no free fallback — they abort via exception, hence raise instead of Firecrawl's None-return). Pool is duck-type compatible with `TavilyClient` (`.search`); one shared pool flows through all company_agent steps so exhaustion state carries across the run; job_agent backfill client replaced too. Same string-matching caveat as BUG-68. | Fixed |
