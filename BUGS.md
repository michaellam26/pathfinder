...
> Last updated: 2026-07-07 (PRJ-004 Phase 4 QA: BUG-62/63 logged from review findings)
...
| BUG-62 | `agents/job_agent.py` `_process_scraped_jd` (empty-company early return) | **Extraction returning empty `company` leaves no audit row** (PRJ-004 Phase 4 code review F5; pre-existing, not introduced by PRJ-004). When Gemini returns data without a `company`, the JD is skipped with only a log line — no `JSON ERROR` row is written, so the URL is retried on every run with no sheet-visible trace. Recommended fix: stage a `_JD_ERROR_ROW`-style audit record (data_quality="failed") after N consecutive empty extractions, so the retry loop becomes observable and bounded. | Open |
