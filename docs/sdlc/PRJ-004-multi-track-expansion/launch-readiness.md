...
|---|------|-------|------------------------------------------|
| 1 | **Prune Company_List** — remove primes / out-of-thesis rows before re-bucketing | **User** | User confirms the 203-row legacy list reflects the intended universe. |
| 2 | **Run `--migrate-tracks` + spot-check** — one-time re-bucketing to the 6-value `Track` taxonomy; grandfather over-quota survivors; flag unconfident rows | **Engineer Lead** runs; **User** spot-checks | Migration audit output reviewed; **zero `UNMIGRATED — manual review` rows remain** (100% valid `Track` values is a hard G1 requirement, not a band); user accepts the re-bucketing sample (R-05). |
...
| 5 | **Live G6a NVIDIA verification** — HARD GATE | **Engineer Lead** runs; **User** reviews | A live NVIDIA Workday run returns **>20 postings** (pagination proven live, not just unit-proxy). BRD-mandated. |
